...
public class IjkMediaMeta {
    ...
    public ArrayList<PacMap> getParcelableArrayList(String key) {
        return null;
    }
    ...
    public static IjkMediaMeta parse(PacMap mediaMeta) {
        if (mediaMeta == null)
            return null;
        ...
        ArrayList<PacMap> streams = meta
                .getParcelableArrayList(IJKM_KEY_STREAMS);
        if (streams == null)
            return meta;

        int index = -1;
        for (PacMap streamBundle : streams) {
            index++;
