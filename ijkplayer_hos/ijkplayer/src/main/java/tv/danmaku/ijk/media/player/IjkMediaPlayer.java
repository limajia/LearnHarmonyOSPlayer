...
import tv.danmaku.ijk.utils.Log;
...
public final class IjkMediaPlayer extends AbstractMediaPlayer {
    ...
    private void initPlayer(IjkLibLoader libLoader) {
        ...
        EventRunner looper;
        if ((looper = EventRunner.current()) != null) {
            mEventHandler = new MyEventHandler(this, looper);
        } else if ((looper = EventRunner.getMainEventRunner()) != null) {
            mEventHandler = new MyEventHandler(this, looper);
        } else {
            mEventHandler = null;
        }
    ...
    /**
     * Sets the {@link SurfaceOps} to use for displaying the video portion of
     * the media.
     * <p>
...
//        }
    ...
    public void setDataSource(FileDescriptor fd)
        ...
        try {
            ReliableFileDescriptor rfd = new ReliableFileDescriptor(fd);
            ReliableFileDescriptor[] piperfds = rfd.createPipe2();
            _setDataSourceFd(piperfds[0].getNativeFd());
        } catch (Exception e) {
            Log.e("nativeijk", "ReliableFileDescriptor error");
        }
    }
...
//        }
    ...
    @Override
    public void release() {
        Log.i("IjkMediaPlayer_java", "当前release:" + mNativeMediaPlayer + " ");
        stayAwake(false);
        updateSurfaceScreenOn();
        resetListeners();
        _release();
    ...
    public PacMap getMediaMeta() {
        return _getMediaMeta();
    }

    private native PacMap _getMediaMeta();
    ...
    private static class MyEventHandler extends EventHandler {
        ...
        public MyEventHandler(IjkMediaPlayer mp, EventRunner looper) {
            super(looper);
        ...
        protected void processEvent(InnerEvent event) {
            ...
            switch (event.eventId) {
                ...
                case MEDIA_BUFFERING_UPDATE:
                    long bufferPosition = InnerEventUtils.getArg1(event);
                    if (bufferPosition < 0) {
                        bufferPosition = 0;
                    }
                ...
                case MEDIA_SET_VIDEO_SIZE:
                    player.mVideoWidth = InnerEventUtils.getArg1(event);
                    player.mVideoHeight = InnerEventUtils.getArg2(event);
                    player.notifyOnVideoSizeChanged(player.mVideoWidth, player.mVideoHeight,
                            player.mVideoSarNum, player.mVideoSarDen);
                    return;

                case MEDIA_ERROR:
//                    DebugLog.e(TAG, "Error (" + event.arg1 + "," + event.arg2 + ")");
                    if (!player.notifyOnError(InnerEventUtils.getArg1(event), InnerEventUtils.getArg2(event))) {
                        player.notifyOnCompletion();
                    }
                    player.stayAwake(false);
                    return;
                ...
                case MEDIA_INFO:
                    switch (InnerEventUtils.getArg1(event)) {
                        case MEDIA_INFO_VIDEO_RENDERING_START:
                            DebugLog.i(TAG, "Info: MEDIA_INFO_VIDEO_RENDERING_START\n");
                            break;
                    }
                    player.notifyOnInfo(InnerEventUtils.getArg1(event), InnerEventUtils.getArg2(event));
                ...
                case MEDIA_TIMED_TEXT:
                    if (event.object == null) {
                        ...
                        IjkTimedText text = new IjkTimedText(new Rect(0, 0, 1, 1), (String) event.object);
                ...
                case MEDIA_SET_VIDEO_SAR:
                    player.mVideoSarNum = InnerEventUtils.getArg1(event);
                    player.mVideoSarDen = InnerEventUtils.getArg2(event);
                    player.notifyOnVideoSizeChanged(player.mVideoWidth, player.mVideoHeight,
                            player.mVideoSarNum, player.mVideoSarDen);
                    break;

                default:
                    DebugLog.e(TAG, "Unknown message type " + event.eventId);
    ...
    private static void postEventFromNative(Object weakThiz, int what,
        ...
        if (mp.mEventHandler != null) {
            //fixme:  发送消息将回调信息通知给上层
//            Message m = mp.mEventHandler.obtainMessage(what, arg1, arg2, obj);
//            mp.mEventHandler.sendMessage(m);
            InnerEvent innerEvent = InnerEvent.get();
            innerEvent.eventId = what;
            innerEvent.object = obj;
            InnerEventUtils.setExInfo(innerEvent, arg1, arg2);
            mp.mEventHandler.sendEvent(innerEvent);
        }
    }
    ...
    public interface OnNativeInvokeListener {
         ...
         */
        boolean onNativeInvoke(int what, PacMap args);
    }

    @CalledByNative
    private static boolean onNativeInvoke(Object weakThiz, int what, PacMap args) {
        // Log.e("ndk","开始执行:"+what+"");
        ...
        switch (what) {
            case OnNativeInvokeListener.CTRL_WILL_CONCAT_RESOLVE_SEGMENT: {
                ...
                int segmentIndex = args.getIntValue(OnNativeInvokeListener.ARG_SEGMENT_INDEX, -1);
                if (segmentIndex < 0)
    ...
    public static class DefaultMediaCodecSelector implements OnMediaCodecSelectListener {
        ...
        public String onMediaCodecSelect(IMediaPlayer mp, String mimeType, int profile, int level) {
            ...
            return "OMX.hisi.video.decoder.avc";
        }
    }
