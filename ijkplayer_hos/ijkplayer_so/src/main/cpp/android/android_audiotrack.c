...
#include "ohos/AudioRendererWrapper.h"
#include "ohos/audio_stream_info.h"
...
{
    jobject volume=J4AC_ohos_media_audio_AudioRenderer__ChannelVolume__ChannelVolume(env,left_volume,right_volume);
    J4AC_ohos_media_audio_AudioRenderer__setVolume(env,atrack->thiz,volume);
    //J4AC_AudioRender(env, atrack->thiz, left_volume, right_volume);
}
...
SDL_Android_AudioTrack * SDL_Android_AudioTrack_new_from_spec(JNIEnv * env, SDL_Android_AudioTrack_Spec * spec)
{
    ...
    int min_buffer_size = J4AC_ohos_media_audio_AudioRenderer__getMinBufferSize__catchAll(env,
            atrack->spec.sample_rate_in_hz,
            androidToOhosEncondingFormat(env, atrack->spec.audio_format),
            androidToOhosChannelMask(env, atrack->spec.channel_config));

    char c[50];
    sprintf(c,"getMinBufferSize=%d",min_buffer_size);
    ...
    jobject audioStreamInfoBuilder = J4AC_ohos_media_audio_AudioStreamInfo__Builder__Builder__catchAll(env);


    J4AC_ohos_media_audio_AudioStreamInfo__Builder__sampleRate(env, audioStreamInfoBuilder, atrack->spec.sample_rate_in_hz);


    J4AC_ohos_media_audio_AudioStreamInfo__Builder__encodingFormat(env, audioStreamInfoBuilder,
            androidToOhosEncondingFormat(env, atrack->spec.audio_format));


    J4AC_ohos_media_audio_AudioStreamInfo__Builder__channelMask(env, audioStreamInfoBuilder,
            androidToOhosChannelMask(env, atrack->spec.channel_config));



    J4AC_ohos_media_audio_AudioStreamInfo__Builder__streamUsage(env, audioStreamInfoBuilder,
            J4AC_AudioStreamInfo_AudioStreamInfo_StreamUsage_STREAM_USAGE_MEDIA(env));



    jobject audioStreamInfo = J4AC_ohos_media_audio_AudioStreamInfo__Builder__build__catchAll(env, audioStreamInfoBuilder);


    jobject audioRenderedInfoBuilder = J4AC_ohos_media_audio_AudioRendererInfo__Builder__Builder__catchAll(env);


    J4AC_ohos_media_audio_AudioRendererInfo__Builder__audioStreamInfo__catchAll(env, audioRenderedInfoBuilder, audioStreamInfo);


    J4AC_ohos_media_audio_AudioRendererInfo__Builder__audioStreamOutputFlag(env,
            audioRenderedInfoBuilder,
            J4AC_AudioStreamInfo_AudioStreamInfo_AudioStreamOutputFlag_AUDIO_STREAM_OUTPUT_FLAG_COMPRESS_OFFLOAD(env));


    J4AC_ohos_media_audio_AudioRendererInfo__Builder__bufferSizeInBytes(env, audioRenderedInfoBuilder, min_buffer_size);


    J4AC_ohos_media_audio_AudioRendererInfo__Builder__isOffload(env, audioRenderedInfoBuilder, true);


    jobject audioRenderInfo = J4AC_ohos_media_audio_AudioRendererInfo__Builder__build(env, audioRenderedInfoBuilder);


    atrack->thiz = J4AC_ohos_media_audio_AudioRenderer__AudioRenderer__asGlobalRef__catchAll(env, audioRenderInfo,
            J4AC_AudioRenderer_PlayMode_MODE_STREAM(env));
...
void SDL_Android_AudioTrack_free(JNIEnv * env, SDL_Android_AudioTrack* atrack)
{
    ...
    if (atrack->thiz) {
        J4AC_ohos_media_audio_AudioRenderer__release(env, atrack->thiz);
...
void SDL_Android_AudioTrack_play(JNIEnv * env, SDL_Android_AudioTrack * atrack)
{
    ...
    J4AC_ohos_media_audio_AudioRenderer__start__catchAll(env, atrack->thiz);
}
...
void SDL_Android_AudioTrack_pause(JNIEnv * env, SDL_Android_AudioTrack * atrack)
{
    ...
    J4AC_ohos_media_audio_AudioRenderer__pause__catchAll(env, atrack->thiz);
}
...
void SDL_Android_AudioTrack_flush(JNIEnv * env, SDL_Android_AudioTrack * atrack)
{
    ...
    J4AC_ohos_media_audio_AudioRenderer__flush__catchAll(env, atrack->thiz);
}

void SDL_Android_AudioTrack_stop(JNIEnv * env, SDL_Android_AudioTrack * atrack)
{
    SDLTRACE("%s", __func__);
    J4AC_ohos_media_audio_AudioRenderer__stop__catchAll(env, atrack->thiz);
}
...
int SDL_Android_AudioTrack_write(JNIEnv * env, SDL_Android_AudioTrack * atrack, uint8_t * data, int size_in_byte)
{
    ...
    int retval = J4AC_ohos_media_audio_AudioRenderer__write__catchAll(env, atrack->thiz, atrack->byte_buffer, 0, (int)size_in_byte);
    if (J4A_ExceptionCheck__catchAll(env))
        return -1;
...
{
    SDLTRACE("%s", __func__);
    int audioSessionId = J4AC_ohos_media_audio_AudioRenderer__getRendererSessionId(env, atrack->thiz);
    if (J4A_ExceptionCheck__catchAll(env))
        return 0;
