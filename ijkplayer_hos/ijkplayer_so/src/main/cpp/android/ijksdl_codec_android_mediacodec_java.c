...
static sdl_amedia_status_t SDL_AMediaCodecJava_configure_surface(JNIEnv* env,
                                                                 ...
                                                                 uint32_t flags)
{
    ...
    J4AC_ohos_media_codec_Codec__setVideoSurface(env, android_media_codec, android_surface);
    J4AC_ohos_media_codec_Codec__setCodecFormat__catchAll(env, android_media_codec, android_media_format);
    J4AC_ohos_media_codec_Codec__registerCodecListener__catchAll(env, android_media_codec, J4AC_ohos_media_mycodec_MyICodecListener__MyICodecListener__catchAll(env));
...
static sdl_amedia_status_t SDL_AMediaCodecJava_start(SDL_AMediaCodec* acodec)
{
    ...
    jboolean started = J4AC_Codec__start(env, android_media_codec);
...
static sdl_amedia_status_t SDL_AMediaCodecJava_stop(SDL_AMediaCodec* acodec)
{
    ...
    jobject android_media_codec = SDL_AMediaCodecJava_getObject(env, acodec);
    J4AC_Codec__stop(env, android_media_codec);
    if (J4A_ExceptionCheck__catchAll(env)) {
...
static sdl_amedia_status_t SDL_AMediaCodecJava_flush(SDL_AMediaCodec* acodec)
{
    ...
    jobject android_media_codec = SDL_AMediaCodecJava_getObject(env, acodec);
    //  J4AC_Codec__flush(env, android_media_codec);
    if (J4A_ExceptionCheck__catchAll(env)) {
...
static ssize_t SDL_AMediaCodecJava_writeInputData(SDL_AMediaCodec* acodec, size_t idx, const uint8_t * data, size_t size, int64_t timeoutUs)
{
    AMCTRACE("%s", __func__);
    ssize_t write_ret = -1;
    ...
    input_buffer_array = J4AC_Codec__getAvailableBuffer__catchAll(env, opaque->android_media_codec, 10);
    ...
    ALOGD("SDL_AMediaCodecJava_writeInputData 数组开始---------------------------- %d",size);
    int tempSize=size>100?100:size;
    char cArray[tempSize*3+50];
    sprintf(cArray,"%s","SDL_AMediaCodecJava_writeInputData");

    for (int i = 0; i < tempSize; i++) {
        // char c[50];
        sprintf(cArray+30+i*3,"%02x ", data[i]&0xff);

    }

    ALOGD("%s",cArray);

    ALOGD("SDL_AMediaCodecJava_writeInputData 数组结束----------------------------");



    //    char cArray[50+tempSize*4];
    //    sprintf(cArray,"%s","SDL_AMediaCodecJava_writeInputData");
    //    char* cArray2=cArray+50;
    //    int count=0;
    ...
    {
        jlong buf_size = (*env)->GetDirectBufferCapacity(env, input_buffer_array);
        void * buf_ptr = (*env)->GetDirectBufferAddress(env, input_buffer_array);

        write_ret = size < buf_size ? size : buf_size;

        memcpy(buf_ptr, data, write_ret);

        jobject bufferInfo = J4AC_ohos_media_common_BufferInfo__BufferInfo(env);

        J4AC_ohos_media_common_BufferInfo__offset__set(env, bufferInfo, 0);
        J4AC_ohos_media_common_BufferInfo__size__set(env, bufferInfo, write_ret);
        J4AC_ohos_media_common_BufferInfo__timeStamp__set(env, bufferInfo, timeoutUs);

        char c2[50];
        sprintf(c2, "bufferInfo offsrt=0,size=%d,timeoutUs=%d", write_ret, timeoutUs);
        ...
        jboolean ret = J4AC_Codec__writeBuffer__catchAll(env, opaque->android_media_codec, input_buffer_array, bufferInfo);
    }
...
ssize_t SDL_AMediaCodecJava_dequeueOutputBuffer(SDL_AMediaCodec* acodec, SDL_AMediaCodecBufferInfo * info, int64_t timeoutUs)
{
    ...
    while (1) {
        idx = 0;//J4AC_MediaCodec__dequeueOutputBuffer(env, android_media_codec, opaque->output_buffer_info, (jlong)timeoutUs);
        if (J4A_ExceptionCheck__catchAll(env)) {
            ALOGI("%s: Exception\n", __func__);
            return AMEDIACODEC__UNKNOWN_ERROR;
        }
...
SDL_AMediaCodec* SDL_AMediaCodecJava_createByCodecName(JNIEnv * env, const char * codec_name)
{
    ...
    jobject android_media_codec = J4AC_Codec__createDecoder(env);
