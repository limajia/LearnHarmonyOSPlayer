...
#include "android/ijksdl_android_jni.h"
#include "ffLogWrapper.h"

#include "mix_compile.h"
...
static IjkMediaPlayer *jni_get_media_player(JNIEnv* env, jobject thiz)
{
    ...
    ALOGD("jni_get_media_player%d",mp);
    if (mp) {
        ijkmp_inc_ref(mp);
    }
...
{
    ALOGD("IjkMediaPlayer_release开始");
    MPTRACE("打印方法%s\n", __func__);
    IjkMediaPlayer *mp = jni_get_media_player(env, thiz);

    if (!mp){
        ALOGD("IjkMediaPlayer_release mp为空%d",mp);
        return;
    }else{
        ALOGD("IjkMediaPlayer_release mp不为空%d",mp);
    }
    ...
    ALOGD("IjkMediaPlayer_release end");
}
...
{
    ALOGD("IjkMediaPlayer_setOption----------------------------");
    MPTRACE("%s\n", __func__);
    IjkMediaPlayer *mp = jni_get_media_player(env, thiz);
...
static bool mediacodec_select_callback(void *opaque, ijkmp_mediacodecinfo_context *mcc)
{
    ...
    if (J4A_ExceptionCheck__catchAll(env) || !found_codec_name) {
        char c[50];
        sprintf(c,"mediacodec_select_callback2 %s",__func__);
...
static JNINativeMethod g_methods[] = {
    ...
    { "_setVideoSurface",       "(Lohos/agp/graphics/Surface;)V", (void *) IjkMediaPlayer_setVideoSurface },
    ...
    { "_getMediaMeta",          "()Lohos/utils/PacMap;",    (void *) IjkMediaPlayer_getMediaMeta },
...
#ifdef MIX_COMPILE
    ALOGD("JNI_OnLoad执行开始");
    startJniOnload(vm,reserved);
#endif
