...
#include "android/ijksdl_codec_android_mediaformat_java.h"
#include "android/ijksdl_codec_android_mediacodec_java.h"
#include "android/ijksdl_codec_android_mediacodec_dummy.h"
...
//            char c[50];
//            sprintf(c, "recreate_format_l SDL_AMediaFormat_setBuffer2 %d %d %d", sps_pps_size,
//                    opaque->nal_size,sps_pps_size);
//

//            for (int i = 0; i < sps_pps_size-10; i++) {
//                char c[70];
//                sprintf(c, "recreate_format_l SDL_AMediaFormat_setBuffer2 %d, %d", i, convert_buffer[i]);
//
//            }
//
//            for (int i = 0; i < 10; i++) {
//                char c[70];
//                sprintf(c, "recreate_format_l SDL_AMediaFormat_setBuffer2 %d, %d", i, (convert_buffer + (sps_pps_size - 10))[i]);
//
//            }


            //SDL_AMediaFormat_setBuffer(opaque->input_aformat, "csd-0", convert_buffer, sps_pps_size - 10);
            //SDL_AMediaFormat_setBuffer(opaque->input_aformat, "csd-1", convert_buffer + (sps_pps_size - 10), 10);
            ...
            restore_mpeg4_esds(opaque->codecpar,
                    opaque->codecpar->extradata,
                    opaque->codecpar->extradata_size,
                    esds_es_dscr_type_length,
                    esds_dec_dscr_type_length,
                    convert_buffer);
...
static int reconfigure_codec_l(JNIEnv * env, IJKFF_Pipenode * node, jobject new_surface)
{
    ...
    ALOGD("reconfigure_codec_l 5");

    ret = 0;

    ALOGD("reconfigure_codec_l 结束");
    fail:
    return ret;
}
...
static int amc_fill_frame(IJKFF_Pipenode            * node,
                          AVFrame                   * frame,
                          int                       * got_frame,
...
#if AMC_USE_AVBITSTREAM_FILTER
#else
        H264ConvertState convert_state = {
            0, 0
        };
#endif
        AVPacket pkt;
...
//        } else {
           // SDL_AMediaCodecFake_flushFakeFrames(opaque->acodec);
            ...
            copy_size = SDL_AMediaCodec_writeInputData(opaque->acodec, input_buffer_index, d->pkt_temp.data, d->pkt_temp.size,time_stamp);
            if (!copy_size) {
